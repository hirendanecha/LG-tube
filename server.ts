...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/lg-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'dist/lg-tube/browser'),
        'index.html'
      )
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        var seo: any = {
          ...
          image:
            'https://www.latinagirls.tube/assets/images/landingpage/OD-default-profile.png',
        ...
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'lg-tube Post',
